...
(function (global) {
    ...
    // ---- DAG (drawn by dag_view.js) ------------------------------------

    function drawDag() {
        const nodes = T.dag.nodes
            .filter(function (n) { return T.nodes[n.id]; })
            .map(function (n) { return { id: n.id, name: n.name }; });
        layout = DagView.render(document.getElementById('dag'), { nodes: nodes, edges: T.dag.edges }, { onClick: openNode });
    }
